...
description: 完整工作流：从 X ID 调研 → 更新 list.yaml → 生成创作者 MD 文件 → 构建发布。覆盖 OnlyFans 博主索引项目的全流程操作。
---
...
**核心原则：**
- X 账号不是必须的。如果未提供 X ID，直接使用创作者名称进行网络搜索
- 调研**不限于 X 平台**，必须从多平台、多引擎综合搜索，找到尽可能多的信息

```
X ID 不为空 ──→ 1.1 获取 X Bio → 1.2 提取名称 → 1.3 网页搜索 → 1.4 平台直连验证
X ID 为空 ────→ 直接进入 1.3 网页搜索
                                    │
                                    ▼
                             1.5 汇总调研结果
```
...
[◀ 返回 <首字母> 列表](./README.md) · [🏠 首页](../index.md)
```
